...
test.describe("quick-open", () => {
  ...
  test("起動直後の空クエリで fixture 全 8 件が候補に並ぶ", async ({ page }) => {
    await waitForAppReady(page);

    await page.keyboard.press(`${modKey}+KeyP`);

    await expect(page.getByPlaceholder("Go to File")).toBeVisible();
    // hot-reload-clipboard.test.ts / rendering.test.ts が並列ワーカーで走ると副 fixture
    // (hot-reload-new-file.md / hot-reload-delete-target.md / rendering-invalid-mermaid.md)
    // が transient に出現する。永続 fixture (hot-reload-target.md / rendering-showcase.md) は
    // 8 件にカウントイン、副 fixture は除外する。
    ...
    // 永続 fixture "hot-reload-target.md" / "rendering-showcase.md" は prefix が異なるため除外されない。
    await expect(
      page.locator('[data-slot="command-item"]').filter({
        hasNotText:
          /^(hot-reload-(new-file|delete-target)|rendering-invalid-mermaid)\.md/,
      })
    ).toHaveCount(8);
  });
