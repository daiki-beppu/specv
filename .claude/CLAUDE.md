...
[takt](https://github.com/nrslib/takt) で issue を実装する。workflow は builtin の `default` / `default-mini` をそのまま使い、プロジェクト固有の eject は行わない（メンテコストを抑えるため）。
...
- workflow を eject せず builtin を使う。upstream で改善されれば自動で恩恵を受ける
